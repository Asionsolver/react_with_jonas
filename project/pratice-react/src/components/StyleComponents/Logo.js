...
export default function Logo() {
  ...
  // pickColor randomly
  // 'LightCoral','IndianRed','LightCoral','Crimson','LightPink','HotPink','DeepPink','MediumVioletRed','OrangeRed','DarkOrange','Violet','Magenta','RebeccaPurple','DarkViolet','Indigo','DarkSlateBlue','DarkMagenta','SpringGreen','DarkBlue','DarkSlateGray' 
  const pickHairColorRandomly = () =>{
    const colors = ['LightCoral','IndianRed','LightCoral','Crimson','LightPink','HotPink','DeepPink','MediumVioletRed','OrangeRed','DarkOrange','Violet','Magenta','RebeccaPurple','DarkViolet','Indigo','DarkSlateBlue','DarkMagenta','SpringGreen','DarkBlue','DarkSlateGray']
    const color = colors[Math.floor(Math.random() * colors.length)]
    return color;
  }
  const pickGlassColorRandomly = () =>{
    const colors = ['LightCoral','IndianRed','LightCoral','Crimson','LightPink','HotPink','DeepPink','MediumVioletRed','OrangeRed','DarkOrange','Violet','Magenta','RebeccaPurple','DarkViolet','Indigo','DarkSlateBlue','DarkMagenta','Maroon','SpringGreen','DarkBlue','DarkSlateGray']
    const color = colors[Math.floor(Math.random() * colors.length)]
    return color;
  }
    ...
    <>
      <div className={style.logo}>
        ...
        <div className={style.glasses}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103.63 42.68">
            <g>
              <g>
                <rect
                className={`${style.coloredLine} border`}
                  x="30.08"
                  y="11.04"
                  width="2.54"
                  height="15.01"
                ...
                />
                <rect
                  className={[style.coloredLine,'border'].join(' ') }
                  x="83.39"
                  y="11.04"
                  width="2.53"
                  height="15.01"
        ...
        <div className={style.play}>
          <button style={{backgroundColor:glassColor}} type="button" onClick={()=>setGlassColor(pickGlassColorRandomly())}>Colorize Glass</button>
          <button style={{backgroundColor:hairColor}} type="button" onClick={()=>setHairColor(pickHairColorRandomly())}>Colorize Hair</button>
          <button  type="button" onClick={resetHandler}>Reset Glass</button>
        </div>
      </div>
    </>
  );
